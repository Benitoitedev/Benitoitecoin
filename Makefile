...
pkgdatadir = $(datadir)/benitoite
pkgincludedir = $(includedir)/benitoite
pkglibdir = $(libdir)/benitoite
pkglibexecdir = $(libexecdir)/benitoite
...
CONFIG_HEADER = $(top_builddir)/src/config/benitoite-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/benitoite-config.h.in \
...
ACLOCAL = ${SHELL} /root/Benitoitecoin/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /root/Benitoitecoin/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /root/Benitoitecoin/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /root/Benitoitecoin/build-aux/missing automake-1.15
AWK = gawk
BDB_CPPFLAGS = 
...
MAINT = 
MAKEINFO = ${SHELL} /root/Benitoitecoin/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
MINIUPNPC_LIBS = 
...
OTOOL64 = 
PACKAGE = benitoite
PACKAGE_BUGREPORT = www.benitoite.org
PACKAGE_NAME = Benitoite Core
PACKAGE_STRING = Benitoite Core 1.0.0
PACKAGE_TARNAME = benitoite
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /root/Benitoitecoin
abs_srcdir = /root/Benitoitecoin
abs_top_builddir = /root/Benitoitecoin
abs_top_srcdir = /root/Benitoitecoin
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /root/Benitoitecoin/build-aux/install-sh
...
SUBDIRS = src
BITCOIND_BIN = $(top_builddir)/src/benitoited$(EXEEXT)
BITCOIN_QT_BIN = $(top_builddir)/src/qt/benitoite-qt$(EXEEXT)
BITCOIN_CLI_BIN = $(top_builddir)/src/benitoite-cli$(EXEEXT)
BITCOIN_WIN_INSTALLER = $(PACKAGE)-$(PACKAGE_VERSION)-win$(WINDOWS_BITS)-setup$(EXEEXT)
empty := 
space := $(empty) $(empty)
OSX_APP = Benitoite-Qt.app
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_benitoite_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_benitoite_coverage.info test_benitoite.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/Benitoite-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/benitoite-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/benitoite-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/benitoite-config.h
$(top_srcdir)/src/config/benitoite-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/benitoite-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/Benitoite-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Benitoite-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Benitoite-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_benitoite.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_benitoite -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_benitoite_filtered.info: test_benitoite.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_benitoite_filtered.info
#	$(MKDIR_P) qa/tmp
...
#test_benitoite_coverage.info: baseline_filtered_combined.info test_benitoite_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_benitoite_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_benitoite_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_benitoite_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_benitoite.coverage/.dirstamp:  test_benitoite_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_benitoite.coverage/.dirstamp total.coverage/.dirstamp

# Benitoite: disable the outdated comparison tool test
...
clean-local:
	rm -rf test_benitoite.coverage/ total.coverage/ $(OSX_APP)
